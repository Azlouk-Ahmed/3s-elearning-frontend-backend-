<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="shortcut icon" type="image" href="img/Layer_2.png">
    
    <title>blog</title>
    <link rel="stylesheet" href="css/normalise.css">
    <link rel="stylesheet" href="css/styles.css">
</head>
<body>
  <div class="progress-wrap float">
		<svg class="progress-circle svg-content" width="100%" height="100%" viewBox="-1 -1 102 102">
			<path d="M50,1 a49,49 0 0,1 0,98 a49,49 0 0,1 0,-98"/>
		</svg>
	</div>
    <div class="header container bg">
        <div>
          <svg
            width="12"
            height="16"
            viewBox="0 0 12 16"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M5.38338 15.6772C0.842813 9.09472 0 8.41916 0 6C0 2.68628 2.68628 0 6 0C9.31372 0 12 2.68628 12 6C12 8.41916 11.1572 9.09472 6.61662 15.6772C6.31866 16.1076 5.68131 16.1076 5.38338 15.6772ZM6 8.5C7.38072 8.5 8.5 7.38072 8.5 6C8.5 4.61928 7.38072 3.5 6 3.5C4.61928 3.5 3.5 4.61928 3.5 6C3.5 7.38072 4.61928 8.5 6 8.5Z"
              fill="#703BF7"
            />
          </svg>
          Menzel Abderrahman Bizerte, 7035
        </div>
        <div class="df">
          <div class="df">
            <svg
              width="17"
              height="16"
              viewBox="0 0 17 16"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M16.2348 11.306L12.7348 9.80597C12.5853 9.74225 12.4192 9.72882 12.2613 9.76771C12.1035 9.8066 11.9626 9.8957 11.8598 10.0216L10.3098 11.9153C7.87726 10.7684 5.91959 8.81073 4.77266 6.37815L6.66641 4.82815C6.79256 4.72555 6.88184 4.58465 6.92075 4.42676C6.95966 4.26888 6.94606 4.10262 6.88203 3.95315L5.38203 0.453154C5.31175 0.292032 5.18746 0.160481 5.03058 0.081185C4.8737 0.00188911 4.69407 -0.0201814 4.52266 0.0187791L1.27266 0.768779C1.1074 0.806941 0.959952 0.899991 0.854386 1.03274C0.74882 1.16549 0.691368 1.33011 0.691406 1.49972C0.691406 9.51534 7.18828 15.9997 15.1914 15.9997C15.3611 15.9998 15.5258 15.9424 15.6586 15.8368C15.7914 15.7313 15.8845 15.5838 15.9227 15.4185L16.6727 12.1685C16.7114 11.9962 16.6888 11.8159 16.6089 11.6585C16.529 11.501 16.3967 11.3764 16.2348 11.306Z"
                fill="#703BF7"
              />
            </svg>
            +216 51 717 722
          </div>
          <div class="df">
            <svg
              width="17"
              height="12"
              viewBox="0 0 17 12"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M15.7555 3.9625C15.8773 3.86563 16.0586 3.95625 16.0586 4.10938V10.5C16.0586 11.3281 15.3867 12 14.5586 12H1.55859C0.730469 12 0.0585938 11.3281 0.0585938 10.5V4.1125C0.0585938 3.95625 0.236719 3.86875 0.361719 3.96562C1.06172 4.50937 1.98984 5.2 5.17734 7.51562C5.83672 7.99687 6.94922 9.00938 8.05859 9.00313C9.17422 9.0125 10.3086 7.97813 10.943 7.51562C14.1305 5.2 15.0555 4.50625 15.7555 3.9625ZM8.05859 8C8.78359 8.0125 9.82734 7.0875 10.3523 6.70625C14.4992 3.69688 14.8148 3.43437 15.7711 2.68437C15.9523 2.54375 16.0586 2.325 16.0586 2.09375V1.5C16.0586 0.671875 15.3867 0 14.5586 0H1.55859C0.730469 0 0.0585938 0.671875 0.0585938 1.5V2.09375C0.0585938 2.325 0.164844 2.54062 0.346094 2.68437C1.30234 3.43125 1.61797 3.69688 5.76484 6.70625C6.28984 7.0875 7.33359 8.0125 8.05859 8Z"
                fill="#703BF7"
              />
            </svg>
            contact@3s-spring.tn
          </div>
        </div>
      </div>
      <nav class="nav--bar df container">
      <img src="img/Layer_2.png" alt="" class="layer" srcset="" />
      <div class="links df" id="links">
        <a href="landing.php">Home</a><a href="about.php">About Us</a><a href="coursedetails.php">Course</a
        ><a href="courses.php">Pages</a><a href="blog.php">Blog</a><a href="contact.php">Contact Us</a>
      </div>
      <div class="search df">
        <input type="text" name="" id="" />
        <svg
          width="56"
          height="56"
          viewBox="0 0 56 56"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
        >
          <rect y="0.5" width="56" height="55" rx="5" fill="#8B54FF" />
          <g clip-path="url(#clip0_10017_2160)">
            <path
              d="M33.75 27.7656C33.75 29.5625 33.1641 31.2422 32.1875 32.5703L37.1094 37.5312C37.6172 38 37.6172 38.8203 37.1094 39.289C36.6406 39.7968 35.8203 39.7968 35.3516 39.289L30.3906 34.3281C29.0625 35.3437 27.3828 35.8906 25.625 35.8906C21.1328 35.8906 17.5 32.2578 17.5 27.7656C17.5 23.3125 21.1328 19.6406 25.625 19.6406C30.0781 19.6406 33.75 23.3125 33.75 27.7656ZM25.625 33.3906C28.7109 33.3906 31.25 30.8906 31.25 27.7656C31.25 24.6797 28.7109 22.1406 25.625 22.1406C22.5 22.1406 20 24.6797 20 27.7656C20 30.8906 22.5 33.3906 25.625 33.3906Z"
              fill="white"
            />
          </g>
          <defs>
            <clipPath id="clip0_10017_2160">
              <rect
                width="21"
                height="21"
                fill="white"
                transform="translate(17.5 19.1406)"
              />
            </clipPath>
          </defs>
        </svg>
      </div>
      <svg width="800px" height="800px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M5 6.5H19V8H5V6.5Z" fill="#703BF7"/>
        <path d="M5 16.5H19V18H5V16.5Z" fill="#703BF7"/>
        <path d="M5 11.5H19V13H5V11.5Z" fill="#703BF7"/>
        </svg>
        <div class="night-mode-button">
          <input type="checkbox" class="checkbox" id="night-mode">
          <label for="night-mode" class="label">
            <svg  class="fa-moon"  version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
     viewBox="0 0 56 56" xml:space="preserve">
    <path style="fill:#A5A5A4;" d="M29,28c0-11.917,7.486-22.112,18-26.147C43.892,0.66,40.523,0,37,0C21.561,0,9,12.561,9,28
    s12.561,28,28,28c3.523,0,6.892-0.66,10-1.853C36.486,50.112,29,39.917,29,28z"/>
    </svg>
    <svg  width="30px" class="fa-sun" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" 
     viewBox="0 0 52 52" xml:space="preserve">
    <ellipse style="fill:#EEAF4B;" cx="26" cy="26" rx="22.5" ry="26"/>
    <path style="fill:#F0C419;" d="M26,48C15.799,48,7.5,38.131,7.5,26S15.799,4,26,4s18.5,9.869,18.5,22S36.201,48,26,48z"/>
    <path style="fill:#F3D55B;" d="M26,35c-2.543,0-5.5-3.932-5.5-9s2.957-9,5.5-9s5.5,3.932,5.5,9S28.543,35,26,35z"/>
    </svg>
            <div class="blob"></div>
          </label>
        </div>
    </nav>
    <div class="page container center">
      <div class="df-c">
        <div class="title">blog</div>
        <a class="title2" href="landing.php">home > blog</a>
      </div>
      <img src="img/dots.png" alt="" class="dots transitions" srcset="" />
  </div>

    <div class="container df ">
        <div class="df jc-c fx-s">


            <div class="df-c right-blog container">
               <img src="img/linkblog.jpg" alt="" srcset="">
               <div class="df">
                   <div>
                     <svg
                       width="9"
                       height="14"
                       viewBox="0 0 9 14"
                       fill="none"
                       xmlns="http://www.w3.org/2000/svg"
                     >
                       <g clip-path="url(#clip0_6051_1523)">
                         <path
                           d="M8.55469 3.58984L6.07031 1.10547C5.77344 0.808594 5.42188 0.660156 5.01562 0.660156H1.5C1.07812 0.675781 0.726562 0.824219 0.445312 1.10547C0.164062 1.38672 0.015625 1.73828 0 2.16016V11.1602C0.015625 11.582 0.164062 11.9336 0.445312 12.2148C0.726562 12.4961 1.07812 12.6445 1.5 12.6602H7.5C7.92188 12.6445 8.27344 12.4961 8.55469 12.2148C8.83594 11.9336 8.98438 11.582 9 11.1602V4.64453C9 4.23828 8.85156 3.88672 8.55469 3.58984ZM8.03906 4.12891C8.11719 4.20703 8.17188 4.30078 8.20312 4.41016H5.625C5.39062 4.39453 5.26562 4.26953 5.25 4.03516V1.45703C5.35938 1.48828 5.45312 1.54297 5.53125 1.62109L8.03906 4.12891ZM8.25 11.1602C8.25 11.3789 8.17969 11.5586 8.03906 11.6992C7.89844 11.8398 7.71875 11.9102 7.5 11.9102H1.5C1.28125 11.9102 1.10156 11.8398 0.960938 11.6992C0.820312 11.5586 0.75 11.3789 0.75 11.1602V2.16016C0.75 1.94141 0.820312 1.76172 0.960938 1.62109C1.10156 1.48047 1.28125 1.41016 1.5 1.41016H4.5V4.03516C4.51562 4.34766 4.625 4.61328 4.82812 4.83203C5.04688 5.03516 5.3125 5.14453 5.625 5.16016H8.25V11.1602Z"
                           fill="#C7C7C7"
                         />
                       </g>
                       <defs>
                         <clipPath id="clip0_6051_1523">
                           <rect
                             width="9"
                             height="13"
                             fill="white"
                             transform="matrix(1 0 0 -1 0 13.1602)"
                           />
                         </clipPath>
                       </defs>
                     </svg>
                     Lesson 8
                   </div>
                   <div>
                     <svg
                       width="11"
                       height="14"
                       viewBox="0 0 11 14"
                       fill="none"
                       xmlns="http://www.w3.org/2000/svg"
                     >
                       <g clip-path="url(#clip0_6051_1528)">
                         <path
                           d="M5.74219 6.66016C6.58594 6.64453 7.29688 6.35547 7.875 5.79297C8.4375 5.21484 8.72656 4.50391 8.74219 3.66016C8.72656 2.81641 8.4375 2.10547 7.875 1.52734C7.29688 0.964844 6.58594 0.675781 5.74219 0.660156C4.89844 0.675781 4.1875 0.964844 3.60938 1.52734C3.04688 2.10547 2.75781 2.81641 2.74219 3.66016C2.75781 4.50391 3.04688 5.21484 3.60938 5.79297C4.1875 6.35547 4.89844 6.64453 5.74219 6.66016ZM5.74219 1.41016C6.38281 1.42578 6.91406 1.64453 7.33594 2.06641C7.75781 2.48828 7.97656 3.01953 7.99219 3.66016C7.97656 4.30078 7.75781 4.83203 7.33594 5.25391C6.91406 5.67578 6.38281 5.89453 5.74219 5.91016C5.10156 5.89453 4.57031 5.67578 4.14844 5.25391C3.72656 4.83203 3.50781 4.30078 3.49219 3.66016C3.50781 3.01953 3.72656 2.48828 4.14844 2.06641C4.57031 1.64453 5.10156 1.42578 5.74219 1.41016ZM6.9375 7.78516H4.54688C3.40625 7.81641 2.45312 8.21484 1.6875 8.98047C0.921875 9.74609 0.523438 10.6992 0.492188 11.8398C0.492188 12.0742 0.570312 12.2695 0.726562 12.4258C0.882812 12.582 1.07812 12.6602 1.3125 12.6602H10.1719C10.4062 12.6602 10.6016 12.582 10.7578 12.4258C10.9141 12.2695 10.9922 12.0742 10.9922 11.8398C10.9609 10.6992 10.5625 9.74609 9.79688 8.98047C9.03125 8.21484 8.07812 7.81641 6.9375 7.78516ZM10.1719 11.9102H1.3125C1.26562 11.9102 1.24219 11.8867 1.24219 11.8398C1.27344 10.9023 1.59375 10.1211 2.20312 9.49609C2.82812 8.88672 3.60938 8.56641 4.54688 8.53516H6.9375C7.875 8.56641 8.65625 8.88672 9.28125 9.49609C9.89062 10.1211 10.2109 10.9023 10.2422 11.8398C10.2422 11.8867 10.2188 11.9102 10.1719 11.9102Z"
                           fill="#C7C7C7"
                         />
                       </g>
                       <defs>
                         <clipPath id="clip0_6051_1528">
                           <rect
                             width="10.5"
                             height="13"
                             fill="white"
                             transform="matrix(1 0 0 -1 0.492188 13.1602)"
                           />
                         </clipPath>
                       </defs>
                     </svg>
                     Students 60+
                   </div>
                 </div>


                 <div class="title">Educate, Empower, Excel: Discover the Power of
                    Learning!</div>

                <p>
                    Uniquely pursue emerging experiences before liemerging content. Efficiently underwhelm customer directed total
linkage after B2C synergy. Dynamically simplify superior human capital whereas efficient infrastructures generate
business web-readiness after wireless outsourcing.
                </p>

                <span>Read More Details</span>
                <img src="img/linkblog.jpg" alt="" srcset="">
                <div class="df">
                    <div>
                      <svg
                        width="9"
                        height="14"
                        viewBox="0 0 9 14"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <g clip-path="url(#clip0_6051_1523)">
                          <path
                            d="M8.55469 3.58984L6.07031 1.10547C5.77344 0.808594 5.42188 0.660156 5.01562 0.660156H1.5C1.07812 0.675781 0.726562 0.824219 0.445312 1.10547C0.164062 1.38672 0.015625 1.73828 0 2.16016V11.1602C0.015625 11.582 0.164062 11.9336 0.445312 12.2148C0.726562 12.4961 1.07812 12.6445 1.5 12.6602H7.5C7.92188 12.6445 8.27344 12.4961 8.55469 12.2148C8.83594 11.9336 8.98438 11.582 9 11.1602V4.64453C9 4.23828 8.85156 3.88672 8.55469 3.58984ZM8.03906 4.12891C8.11719 4.20703 8.17188 4.30078 8.20312 4.41016H5.625C5.39062 4.39453 5.26562 4.26953 5.25 4.03516V1.45703C5.35938 1.48828 5.45312 1.54297 5.53125 1.62109L8.03906 4.12891ZM8.25 11.1602C8.25 11.3789 8.17969 11.5586 8.03906 11.6992C7.89844 11.8398 7.71875 11.9102 7.5 11.9102H1.5C1.28125 11.9102 1.10156 11.8398 0.960938 11.6992C0.820312 11.5586 0.75 11.3789 0.75 11.1602V2.16016C0.75 1.94141 0.820312 1.76172 0.960938 1.62109C1.10156 1.48047 1.28125 1.41016 1.5 1.41016H4.5V4.03516C4.51562 4.34766 4.625 4.61328 4.82812 4.83203C5.04688 5.03516 5.3125 5.14453 5.625 5.16016H8.25V11.1602Z"
                            fill="#C7C7C7"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_6051_1523">
                            <rect
                              width="9"
                              height="13"
                              fill="white"
                              transform="matrix(1 0 0 -1 0 13.1602)"
                            />
                          </clipPath>
                        </defs>
                      </svg>
                      Lesson 8
                    </div>
                    <div>
                      <svg
                        width="11"
                        height="14"
                        viewBox="0 0 11 14"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <g clip-path="url(#clip0_6051_1528)">
                          <path
                            d="M5.74219 6.66016C6.58594 6.64453 7.29688 6.35547 7.875 5.79297C8.4375 5.21484 8.72656 4.50391 8.74219 3.66016C8.72656 2.81641 8.4375 2.10547 7.875 1.52734C7.29688 0.964844 6.58594 0.675781 5.74219 0.660156C4.89844 0.675781 4.1875 0.964844 3.60938 1.52734C3.04688 2.10547 2.75781 2.81641 2.74219 3.66016C2.75781 4.50391 3.04688 5.21484 3.60938 5.79297C4.1875 6.35547 4.89844 6.64453 5.74219 6.66016ZM5.74219 1.41016C6.38281 1.42578 6.91406 1.64453 7.33594 2.06641C7.75781 2.48828 7.97656 3.01953 7.99219 3.66016C7.97656 4.30078 7.75781 4.83203 7.33594 5.25391C6.91406 5.67578 6.38281 5.89453 5.74219 5.91016C5.10156 5.89453 4.57031 5.67578 4.14844 5.25391C3.72656 4.83203 3.50781 4.30078 3.49219 3.66016C3.50781 3.01953 3.72656 2.48828 4.14844 2.06641C4.57031 1.64453 5.10156 1.42578 5.74219 1.41016ZM6.9375 7.78516H4.54688C3.40625 7.81641 2.45312 8.21484 1.6875 8.98047C0.921875 9.74609 0.523438 10.6992 0.492188 11.8398C0.492188 12.0742 0.570312 12.2695 0.726562 12.4258C0.882812 12.582 1.07812 12.6602 1.3125 12.6602H10.1719C10.4062 12.6602 10.6016 12.582 10.7578 12.4258C10.9141 12.2695 10.9922 12.0742 10.9922 11.8398C10.9609 10.6992 10.5625 9.74609 9.79688 8.98047C9.03125 8.21484 8.07812 7.81641 6.9375 7.78516ZM10.1719 11.9102H1.3125C1.26562 11.9102 1.24219 11.8867 1.24219 11.8398C1.27344 10.9023 1.59375 10.1211 2.20312 9.49609C2.82812 8.88672 3.60938 8.56641 4.54688 8.53516H6.9375C7.875 8.56641 8.65625 8.88672 9.28125 9.49609C9.89062 10.1211 10.2109 10.9023 10.2422 11.8398C10.2422 11.8867 10.2188 11.9102 10.1719 11.9102Z"
                            fill="#C7C7C7"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_6051_1528">
                            <rect
                              width="10.5"
                              height="13"
                              fill="white"
                              transform="matrix(1 0 0 -1 0.492188 13.1602)"
                            />
                          </clipPath>
                        </defs>
                      </svg>
                      Students 60+
                    </div>
                  </div>
                <div class="title">Enrich Your Mind Envision Your Future Education
                    for Success</div>

                <p>
                    Embrace the world of online education with our resources on virtual classrooms, e-learning platforms, and MOOCs
(Massive Open Online Courses). Learn how to navigate online courses effectively and make the most of digital
learning tools.Discover a curated list of educational apps.
                </p>
                <span>Read More Details</span>


                <div class="df">
                    <div>
                      <svg
                        width="9"
                        height="14"
                        viewBox="0 0 9 14"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <g clip-path="url(#clip0_6051_1523)">
                          <path
                            d="M8.55469 3.58984L6.07031 1.10547C5.77344 0.808594 5.42188 0.660156 5.01562 0.660156H1.5C1.07812 0.675781 0.726562 0.824219 0.445312 1.10547C0.164062 1.38672 0.015625 1.73828 0 2.16016V11.1602C0.015625 11.582 0.164062 11.9336 0.445312 12.2148C0.726562 12.4961 1.07812 12.6445 1.5 12.6602H7.5C7.92188 12.6445 8.27344 12.4961 8.55469 12.2148C8.83594 11.9336 8.98438 11.582 9 11.1602V4.64453C9 4.23828 8.85156 3.88672 8.55469 3.58984ZM8.03906 4.12891C8.11719 4.20703 8.17188 4.30078 8.20312 4.41016H5.625C5.39062 4.39453 5.26562 4.26953 5.25 4.03516V1.45703C5.35938 1.48828 5.45312 1.54297 5.53125 1.62109L8.03906 4.12891ZM8.25 11.1602C8.25 11.3789 8.17969 11.5586 8.03906 11.6992C7.89844 11.8398 7.71875 11.9102 7.5 11.9102H1.5C1.28125 11.9102 1.10156 11.8398 0.960938 11.6992C0.820312 11.5586 0.75 11.3789 0.75 11.1602V2.16016C0.75 1.94141 0.820312 1.76172 0.960938 1.62109C1.10156 1.48047 1.28125 1.41016 1.5 1.41016H4.5V4.03516C4.51562 4.34766 4.625 4.61328 4.82812 4.83203C5.04688 5.03516 5.3125 5.14453 5.625 5.16016H8.25V11.1602Z"
                            fill="#C7C7C7"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_6051_1523">
                            <rect
                              width="9"
                              height="13"
                              fill="white"
                              transform="matrix(1 0 0 -1 0 13.1602)"
                            />
                          </clipPath>
                        </defs>
                      </svg>
                      Lesson 8
                    </div>
                    <div>
                      <svg
                        width="11"
                        height="14"
                        viewBox="0 0 11 14"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <g clip-path="url(#clip0_6051_1528)">
                          <path
                            d="M5.74219 6.66016C6.58594 6.64453 7.29688 6.35547 7.875 5.79297C8.4375 5.21484 8.72656 4.50391 8.74219 3.66016C8.72656 2.81641 8.4375 2.10547 7.875 1.52734C7.29688 0.964844 6.58594 0.675781 5.74219 0.660156C4.89844 0.675781 4.1875 0.964844 3.60938 1.52734C3.04688 2.10547 2.75781 2.81641 2.74219 3.66016C2.75781 4.50391 3.04688 5.21484 3.60938 5.79297C4.1875 6.35547 4.89844 6.64453 5.74219 6.66016ZM5.74219 1.41016C6.38281 1.42578 6.91406 1.64453 7.33594 2.06641C7.75781 2.48828 7.97656 3.01953 7.99219 3.66016C7.97656 4.30078 7.75781 4.83203 7.33594 5.25391C6.91406 5.67578 6.38281 5.89453 5.74219 5.91016C5.10156 5.89453 4.57031 5.67578 4.14844 5.25391C3.72656 4.83203 3.50781 4.30078 3.49219 3.66016C3.50781 3.01953 3.72656 2.48828 4.14844 2.06641C4.57031 1.64453 5.10156 1.42578 5.74219 1.41016ZM6.9375 7.78516H4.54688C3.40625 7.81641 2.45312 8.21484 1.6875 8.98047C0.921875 9.74609 0.523438 10.6992 0.492188 11.8398C0.492188 12.0742 0.570312 12.2695 0.726562 12.4258C0.882812 12.582 1.07812 12.6602 1.3125 12.6602H10.1719C10.4062 12.6602 10.6016 12.582 10.7578 12.4258C10.9141 12.2695 10.9922 12.0742 10.9922 11.8398C10.9609 10.6992 10.5625 9.74609 9.79688 8.98047C9.03125 8.21484 8.07812 7.81641 6.9375 7.78516ZM10.1719 11.9102H1.3125C1.26562 11.9102 1.24219 11.8867 1.24219 11.8398C1.27344 10.9023 1.59375 10.1211 2.20312 9.49609C2.82812 8.88672 3.60938 8.56641 4.54688 8.53516H6.9375C7.875 8.56641 8.65625 8.88672 9.28125 9.49609C9.89062 10.1211 10.2109 10.9023 10.2422 11.8398C10.2422 11.8867 10.2188 11.9102 10.1719 11.9102Z"
                            fill="#C7C7C7"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_6051_1528">
                            <rect
                              width="10.5"
                              height="13"
                              fill="white"
                              transform="matrix(1 0 0 -1 0.492188 13.1602)"
                            />
                          </clipPath>
                        </defs>
                      </svg>
                      Students 60+
                    </div>
                  </div>

                <div class="title">
                    Gain insights into how parents can support childs
Education at home
                </div>

                <p>
                    Access a range of teaching strategies, lesson plans, classroom management techniques, and assessment methods
for educators. Stay updated on the latest trends and best practices in education. Explore profiles of professionals in
various fields to gain insights into their experiences and career journeys.
                </p>

                <span>
                    Read More Details
                </span>

                <img src="img/blogvid.jpg" alt="" srcset="">
                <div class="df">
                    <div>
                      <svg
                        width="9"
                        height="14"
                        viewBox="0 0 9 14"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <g clip-path="url(#clip0_6051_1523)">
                          <path
                            d="M8.55469 3.58984L6.07031 1.10547C5.77344 0.808594 5.42188 0.660156 5.01562 0.660156H1.5C1.07812 0.675781 0.726562 0.824219 0.445312 1.10547C0.164062 1.38672 0.015625 1.73828 0 2.16016V11.1602C0.015625 11.582 0.164062 11.9336 0.445312 12.2148C0.726562 12.4961 1.07812 12.6445 1.5 12.6602H7.5C7.92188 12.6445 8.27344 12.4961 8.55469 12.2148C8.83594 11.9336 8.98438 11.582 9 11.1602V4.64453C9 4.23828 8.85156 3.88672 8.55469 3.58984ZM8.03906 4.12891C8.11719 4.20703 8.17188 4.30078 8.20312 4.41016H5.625C5.39062 4.39453 5.26562 4.26953 5.25 4.03516V1.45703C5.35938 1.48828 5.45312 1.54297 5.53125 1.62109L8.03906 4.12891ZM8.25 11.1602C8.25 11.3789 8.17969 11.5586 8.03906 11.6992C7.89844 11.8398 7.71875 11.9102 7.5 11.9102H1.5C1.28125 11.9102 1.10156 11.8398 0.960938 11.6992C0.820312 11.5586 0.75 11.3789 0.75 11.1602V2.16016C0.75 1.94141 0.820312 1.76172 0.960938 1.62109C1.10156 1.48047 1.28125 1.41016 1.5 1.41016H4.5V4.03516C4.51562 4.34766 4.625 4.61328 4.82812 4.83203C5.04688 5.03516 5.3125 5.14453 5.625 5.16016H8.25V11.1602Z"
                            fill="#C7C7C7"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_6051_1523">
                            <rect
                              width="9"
                              height="13"
                              fill="white"
                              transform="matrix(1 0 0 -1 0 13.1602)"
                            />
                          </clipPath>
                        </defs>
                      </svg>
                      Lesson 8
                    </div>
                    <div>
                      <svg
                        width="11"
                        height="14"
                        viewBox="0 0 11 14"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <g clip-path="url(#clip0_6051_1528)">
                          <path
                            d="M5.74219 6.66016C6.58594 6.64453 7.29688 6.35547 7.875 5.79297C8.4375 5.21484 8.72656 4.50391 8.74219 3.66016C8.72656 2.81641 8.4375 2.10547 7.875 1.52734C7.29688 0.964844 6.58594 0.675781 5.74219 0.660156C4.89844 0.675781 4.1875 0.964844 3.60938 1.52734C3.04688 2.10547 2.75781 2.81641 2.74219 3.66016C2.75781 4.50391 3.04688 5.21484 3.60938 5.79297C4.1875 6.35547 4.89844 6.64453 5.74219 6.66016ZM5.74219 1.41016C6.38281 1.42578 6.91406 1.64453 7.33594 2.06641C7.75781 2.48828 7.97656 3.01953 7.99219 3.66016C7.97656 4.30078 7.75781 4.83203 7.33594 5.25391C6.91406 5.67578 6.38281 5.89453 5.74219 5.91016C5.10156 5.89453 4.57031 5.67578 4.14844 5.25391C3.72656 4.83203 3.50781 4.30078 3.49219 3.66016C3.50781 3.01953 3.72656 2.48828 4.14844 2.06641C4.57031 1.64453 5.10156 1.42578 5.74219 1.41016ZM6.9375 7.78516H4.54688C3.40625 7.81641 2.45312 8.21484 1.6875 8.98047C0.921875 9.74609 0.523438 10.6992 0.492188 11.8398C0.492188 12.0742 0.570312 12.2695 0.726562 12.4258C0.882812 12.582 1.07812 12.6602 1.3125 12.6602H10.1719C10.4062 12.6602 10.6016 12.582 10.7578 12.4258C10.9141 12.2695 10.9922 12.0742 10.9922 11.8398C10.9609 10.6992 10.5625 9.74609 9.79688 8.98047C9.03125 8.21484 8.07812 7.81641 6.9375 7.78516ZM10.1719 11.9102H1.3125C1.26562 11.9102 1.24219 11.8867 1.24219 11.8398C1.27344 10.9023 1.59375 10.1211 2.20312 9.49609C2.82812 8.88672 3.60938 8.56641 4.54688 8.53516H6.9375C7.875 8.56641 8.65625 8.88672 9.28125 9.49609C9.89062 10.1211 10.2109 10.9023 10.2422 11.8398C10.2422 11.8867 10.2188 11.9102 10.1719 11.9102Z"
                            fill="#C7C7C7"
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_6051_1528">
                            <rect
                              width="10.5"
                              height="13"
                              fill="white"
                              transform="matrix(1 0 0 -1 0.492188 13.1602)"
                            />
                          </clipPath>
                        </defs>
                      </svg>
                      Students 60+
                    </div>
                  </div>

                <div class="title">University class starting soon while the lovely
                    valley team work</div>

                <p>
                    Explore our collection of articles, tutorials, videos, and interactive tools to enhance your knowledge and foster a love
for learning. Get valuable tips on preparing for exams, including creating study schedules, practicing past papers,
managing exam anxiety, and optimizing your performance on test day.
                </p>

                <span>Read More Details</span>

                <div class="df">
                    <div class="badg-box ta-c"><p>01</p></div>
                    <div class="badg-box ta-c"><p>02</p></div>
                    <div class="badg-box ta-c"><p>03</p></div>
                    <div class="badg-box ta-c"><svg width="15" height="13" viewBox="0 0 15 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M8.78125 0.890625L14.2812 6.14062C14.4271 6.28646 14.5 6.46354 14.5 6.67188C14.5 6.88021 14.4271 7.05729 14.2812 7.20312L8.78125 12.4531C8.40625 12.7448 8.05208 12.7448 7.71875 12.4531C7.42708 12.0781 7.42708 11.724 7.71875 11.3906L11.875 7.42188H1.25C0.791667 7.38021 0.541667 7.13021 0.5 6.67188C0.541667 6.21354 0.791667 5.96354 1.25 5.92188H11.875L7.71875 1.95312C7.42708 1.61979 7.42708 1.26562 7.71875 0.890625C8.05208 0.598958 8.40625 0.598958 8.78125 0.890625Z" fill="#0F2239"/>
                        </svg>
                        </div>
                </div>
            </div>


















            <div class="df-c left-blog">
                <div class="search px3 df">
                    <input type="text" name="" id="" />
                    <svg
                      width="56"
                      height="56"
                      viewBox="0 0 56 56"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <rect y="0.5" width="56" height="55" rx="5" fill="#8B54FF" />
                      <g clip-path="url(#clip0_10017_2160)">
                        <path
                          d="M33.75 27.7656C33.75 29.5625 33.1641 31.2422 32.1875 32.5703L37.1094 37.5312C37.6172 38 37.6172 38.8203 37.1094 39.289C36.6406 39.7968 35.8203 39.7968 35.3516 39.289L30.3906 34.3281C29.0625 35.3437 27.3828 35.8906 25.625 35.8906C21.1328 35.8906 17.5 32.2578 17.5 27.7656C17.5 23.3125 21.1328 19.6406 25.625 19.6406C30.0781 19.6406 33.75 23.3125 33.75 27.7656ZM25.625 33.3906C28.7109 33.3906 31.25 30.8906 31.25 27.7656C31.25 24.6797 28.7109 22.1406 25.625 22.1406C22.5 22.1406 20 24.6797 20 27.7656C20 30.8906 22.5 33.3906 25.625 33.3906Z"
                          fill="white"
                        />
                      </g>
                      <defs>
                        <clipPath id="clip0_10017_2160">
                          <rect
                            width="21"
                            height="21"
                            fill="white"
                            transform="translate(17.5 19.1406)"
                          />
                        </clipPath>
                      </defs>
                    </svg>
                  </div>
                <div class="spacer"></div>
                <div class="spacer"></div>
                <div class="title px3">categories</div>
                <div class="px3 df-c">
                    <div class="df"><svg width="12" height="14" viewBox="0 0 12 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.3602 9.7375C11.3602 9.9375 11.2602 10.1125 11.1352 10.2125C11.0102 10.5875 11.0102 11.6875 11.1352 12.0875C11.2602 12.1875 11.3602 12.3625 11.3602 12.5375V12.9375C11.3602 13.2875 11.0852 13.5375 10.7602 13.5375H2.56016C1.23516 13.5375 0.160157 12.4625 0.160157 11.1375V3.1375C0.160157 1.8125 1.23516 0.7375 2.56016 0.7375H10.7602C11.0852 0.7375 11.3602 1.0125 11.3602 1.3375V9.7375ZM3.36016 4.0875V4.5875C3.36016 4.6875 3.41016 4.7375 3.51016 4.7375H8.81016C8.88516 4.7375 8.96016 4.6875 8.96016 4.5875V4.0875C8.96016 4.0125 8.88516 3.9375 8.81016 3.9375H3.51016C3.41016 3.9375 3.36016 4.0125 3.36016 4.0875ZM3.36016 5.6875V6.1875C3.36016 6.2875 3.41016 6.3375 3.51016 6.3375H8.81016C8.88516 6.3375 8.96016 6.2875 8.96016 6.1875V5.6875C8.96016 5.6125 8.88516 5.5375 8.81016 5.5375H3.51016C3.41016 5.5375 3.36016 5.6125 3.36016 5.6875ZM9.68516 11.9375C9.63516 11.5125 9.63516 10.7875 9.68516 10.3375H2.56016C2.11016 10.3375 1.76016 10.7125 1.76016 11.1375C1.76016 11.5875 2.11016 11.9375 2.56016 11.9375H9.68516Z" fill="#8B54FF"/>
                        </svg>
                        Design
                        </div>
                    <div class="df"><svg width="12" height="14" viewBox="0 0 12 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.3602 9.7375C11.3602 9.9375 11.2602 10.1125 11.1352 10.2125C11.0102 10.5875 11.0102 11.6875 11.1352 12.0875C11.2602 12.1875 11.3602 12.3625 11.3602 12.5375V12.9375C11.3602 13.2875 11.0852 13.5375 10.7602 13.5375H2.56016C1.23516 13.5375 0.160157 12.4625 0.160157 11.1375V3.1375C0.160157 1.8125 1.23516 0.7375 2.56016 0.7375H10.7602C11.0852 0.7375 11.3602 1.0125 11.3602 1.3375V9.7375ZM3.36016 4.0875V4.5875C3.36016 4.6875 3.41016 4.7375 3.51016 4.7375H8.81016C8.88516 4.7375 8.96016 4.6875 8.96016 4.5875V4.0875C8.96016 4.0125 8.88516 3.9375 8.81016 3.9375H3.51016C3.41016 3.9375 3.36016 4.0125 3.36016 4.0875ZM3.36016 5.6875V6.1875C3.36016 6.2875 3.41016 6.3375 3.51016 6.3375H8.81016C8.88516 6.3375 8.96016 6.2875 8.96016 6.1875V5.6875C8.96016 5.6125 8.88516 5.5375 8.81016 5.5375H3.51016C3.41016 5.5375 3.36016 5.6125 3.36016 5.6875ZM9.68516 11.9375C9.63516 11.5125 9.63516 10.7875 9.68516 10.3375H2.56016C2.11016 10.3375 1.76016 10.7125 1.76016 11.1375C1.76016 11.5875 2.11016 11.9375 2.56016 11.9375H9.68516Z" fill="#8B54FF"/>
                        </svg>
                        Development
                        </div>
                    <div class="df"><svg width="12" height="14" viewBox="0 0 12 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.3602 9.7375C11.3602 9.9375 11.2602 10.1125 11.1352 10.2125C11.0102 10.5875 11.0102 11.6875 11.1352 12.0875C11.2602 12.1875 11.3602 12.3625 11.3602 12.5375V12.9375C11.3602 13.2875 11.0852 13.5375 10.7602 13.5375H2.56016C1.23516 13.5375 0.160157 12.4625 0.160157 11.1375V3.1375C0.160157 1.8125 1.23516 0.7375 2.56016 0.7375H10.7602C11.0852 0.7375 11.3602 1.0125 11.3602 1.3375V9.7375ZM3.36016 4.0875V4.5875C3.36016 4.6875 3.41016 4.7375 3.51016 4.7375H8.81016C8.88516 4.7375 8.96016 4.6875 8.96016 4.5875V4.0875C8.96016 4.0125 8.88516 3.9375 8.81016 3.9375H3.51016C3.41016 3.9375 3.36016 4.0125 3.36016 4.0875ZM3.36016 5.6875V6.1875C3.36016 6.2875 3.41016 6.3375 3.51016 6.3375H8.81016C8.88516 6.3375 8.96016 6.2875 8.96016 6.1875V5.6875C8.96016 5.6125 8.88516 5.5375 8.81016 5.5375H3.51016C3.41016 5.5375 3.36016 5.6125 3.36016 5.6875ZM9.68516 11.9375C9.63516 11.5125 9.63516 10.7875 9.68516 10.3375H2.56016C2.11016 10.3375 1.76016 10.7125 1.76016 11.1375C1.76016 11.5875 2.11016 11.9375 2.56016 11.9375H9.68516Z" fill="#8B54FF"/>
                        </svg>
                        Photography
                        </div>
                    <div class="df"><svg width="12" height="14" viewBox="0 0 12 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.3602 9.7375C11.3602 9.9375 11.2602 10.1125 11.1352 10.2125C11.0102 10.5875 11.0102 11.6875 11.1352 12.0875C11.2602 12.1875 11.3602 12.3625 11.3602 12.5375V12.9375C11.3602 13.2875 11.0852 13.5375 10.7602 13.5375H2.56016C1.23516 13.5375 0.160157 12.4625 0.160157 11.1375V3.1375C0.160157 1.8125 1.23516 0.7375 2.56016 0.7375H10.7602C11.0852 0.7375 11.3602 1.0125 11.3602 1.3375V9.7375ZM3.36016 4.0875V4.5875C3.36016 4.6875 3.41016 4.7375 3.51016 4.7375H8.81016C8.88516 4.7375 8.96016 4.6875 8.96016 4.5875V4.0875C8.96016 4.0125 8.88516 3.9375 8.81016 3.9375H3.51016C3.41016 3.9375 3.36016 4.0125 3.36016 4.0875ZM3.36016 5.6875V6.1875C3.36016 6.2875 3.41016 6.3375 3.51016 6.3375H8.81016C8.88516 6.3375 8.96016 6.2875 8.96016 6.1875V5.6875C8.96016 5.6125 8.88516 5.5375 8.81016 5.5375H3.51016C3.41016 5.5375 3.36016 5.6125 3.36016 5.6875ZM9.68516 11.9375C9.63516 11.5125 9.63516 10.7875 9.68516 10.3375H2.56016C2.11016 10.3375 1.76016 10.7125 1.76016 11.1375C1.76016 11.5875 2.11016 11.9375 2.56016 11.9375H9.68516Z" fill="#8B54FF"/>
                        </svg>
                        Health
                        </div>
                    <div class="df"><svg width="12" height="14" viewBox="0 0 12 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.3602 9.7375C11.3602 9.9375 11.2602 10.1125 11.1352 10.2125C11.0102 10.5875 11.0102 11.6875 11.1352 12.0875C11.2602 12.1875 11.3602 12.3625 11.3602 12.5375V12.9375C11.3602 13.2875 11.0852 13.5375 10.7602 13.5375H2.56016C1.23516 13.5375 0.160157 12.4625 0.160157 11.1375V3.1375C0.160157 1.8125 1.23516 0.7375 2.56016 0.7375H10.7602C11.0852 0.7375 11.3602 1.0125 11.3602 1.3375V9.7375ZM3.36016 4.0875V4.5875C3.36016 4.6875 3.41016 4.7375 3.51016 4.7375H8.81016C8.88516 4.7375 8.96016 4.6875 8.96016 4.5875V4.0875C8.96016 4.0125 8.88516 3.9375 8.81016 3.9375H3.51016C3.41016 3.9375 3.36016 4.0125 3.36016 4.0875ZM3.36016 5.6875V6.1875C3.36016 6.2875 3.41016 6.3375 3.51016 6.3375H8.81016C8.88516 6.3375 8.96016 6.2875 8.96016 6.1875V5.6875C8.96016 5.6125 8.88516 5.5375 8.81016 5.5375H3.51016C3.41016 5.5375 3.36016 5.6125 3.36016 5.6875ZM9.68516 11.9375C9.63516 11.5125 9.63516 10.7875 9.68516 10.3375H2.56016C2.11016 10.3375 1.76016 10.7125 1.76016 11.1375C1.76016 11.5875 2.11016 11.9375 2.56016 11.9375H9.68516Z" fill="#8B54FF"/>
                        </svg>
                        Health
                        </div>
                    <div class="df"><svg width="12" height="14" viewBox="0 0 12 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.3602 9.7375C11.3602 9.9375 11.2602 10.1125 11.1352 10.2125C11.0102 10.5875 11.0102 11.6875 11.1352 12.0875C11.2602 12.1875 11.3602 12.3625 11.3602 12.5375V12.9375C11.3602 13.2875 11.0852 13.5375 10.7602 13.5375H2.56016C1.23516 13.5375 0.160157 12.4625 0.160157 11.1375V3.1375C0.160157 1.8125 1.23516 0.7375 2.56016 0.7375H10.7602C11.0852 0.7375 11.3602 1.0125 11.3602 1.3375V9.7375ZM3.36016 4.0875V4.5875C3.36016 4.6875 3.41016 4.7375 3.51016 4.7375H8.81016C8.88516 4.7375 8.96016 4.6875 8.96016 4.5875V4.0875C8.96016 4.0125 8.88516 3.9375 8.81016 3.9375H3.51016C3.41016 3.9375 3.36016 4.0125 3.36016 4.0875ZM3.36016 5.6875V6.1875C3.36016 6.2875 3.41016 6.3375 3.51016 6.3375H8.81016C8.88516 6.3375 8.96016 6.2875 8.96016 6.1875V5.6875C8.96016 5.6125 8.88516 5.5375 8.81016 5.5375H3.51016C3.41016 5.5375 3.36016 5.6125 3.36016 5.6875ZM9.68516 11.9375C9.63516 11.5125 9.63516 10.7875 9.68516 10.3375H2.56016C2.11016 10.3375 1.76016 10.7125 1.76016 11.1375C1.76016 11.5875 2.11016 11.9375 2.56016 11.9375H9.68516Z" fill="#8B54FF"/>
                        </svg>
                        Finance
                        </div>
                    <div class="df"><svg width="12" height="14" viewBox="0 0 12 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M11.3602 9.7375C11.3602 9.9375 11.2602 10.1125 11.1352 10.2125C11.0102 10.5875 11.0102 11.6875 11.1352 12.0875C11.2602 12.1875 11.3602 12.3625 11.3602 12.5375V12.9375C11.3602 13.2875 11.0852 13.5375 10.7602 13.5375H2.56016C1.23516 13.5375 0.160157 12.4625 0.160157 11.1375V3.1375C0.160157 1.8125 1.23516 0.7375 2.56016 0.7375H10.7602C11.0852 0.7375 11.3602 1.0125 11.3602 1.3375V9.7375ZM3.36016 4.0875V4.5875C3.36016 4.6875 3.41016 4.7375 3.51016 4.7375H8.81016C8.88516 4.7375 8.96016 4.6875 8.96016 4.5875V4.0875C8.96016 4.0125 8.88516 3.9375 8.81016 3.9375H3.51016C3.41016 3.9375 3.36016 4.0125 3.36016 4.0875ZM3.36016 5.6875V6.1875C3.36016 6.2875 3.41016 6.3375 3.51016 6.3375H8.81016C8.88516 6.3375 8.96016 6.2875 8.96016 6.1875V5.6875C8.96016 5.6125 8.88516 5.5375 8.81016 5.5375H3.51016C3.41016 5.5375 3.36016 5.6125 3.36016 5.6875ZM9.68516 11.9375C9.63516 11.5125 9.63516 10.7875 9.68516 10.3375H2.56016C2.11016 10.3375 1.76016 10.7125 1.76016 11.1375C1.76016 11.5875 2.11016 11.9375 2.56016 11.9375H9.68516Z" fill="#8B54FF"/>
                        </svg>
                        Technology
                    </div>

                </div>
                <div class="spacer"></div>
                <div class="spacer"></div>
                <div class="spacer"></div>
                <div class="spacer"></div>
                <div class="df-c px3">
                    <div class="title">Recent posts</div>
                    <div class="df">
                        <img src="img/blog1.jpg" class="blogimg" alt="" srcset="">
                        <div class="df-c g-0">
                            <div class="title2">
                                How To Start Learn Online
                                Study From Your Home
                            </div>
                            <div class="df"><svg width="15" height="17" viewBox="0 0 15 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_10099_272)">
                                <path d="M4.16016 0.667969C4.47266 0.688802 4.63932 0.855469 4.66016 1.16797V2.66797H10.6602V1.16797C10.681 0.855469 10.8477 0.688802 11.1602 0.667969C11.4727 0.688802 11.6393 0.855469 11.6602 1.16797V2.66797H12.6602C13.2227 2.6888 13.6914 2.88672 14.0664 3.26172C14.4414 3.63672 14.6393 4.10547 14.6602 4.66797V14.668C14.6393 15.2305 14.4414 15.6992 14.0664 16.0742C13.6914 16.4492 13.2227 16.6471 12.6602 16.668H2.66016C2.09766 16.6471 1.62891 16.4492 1.25391 16.0742C0.878906 15.6992 0.68099 15.2305 0.660156 14.668V4.66797C0.68099 4.10547 0.878906 3.63672 1.25391 3.26172C1.62891 2.88672 2.09766 2.6888 2.66016 2.66797H3.66016V1.16797C3.68099 0.855469 3.84766 0.688802 4.16016 0.667969ZM13.6602 6.66797H1.66016V14.668C1.66016 14.9596 1.75391 15.1992 1.94141 15.3867C2.12891 15.5742 2.36849 15.668 2.66016 15.668H12.6602C12.9518 15.668 13.1914 15.5742 13.3789 15.3867C13.5664 15.1992 13.6602 14.9596 13.6602 14.668V6.66797ZM12.6602 3.66797H2.66016C2.36849 3.66797 2.12891 3.76172 1.94141 3.94922C1.75391 4.13672 1.66016 4.3763 1.66016 4.66797V5.66797H13.6602V4.66797C13.6602 4.3763 13.5664 4.13672 13.3789 3.94922C13.1914 3.76172 12.9518 3.66797 12.6602 3.66797Z" fill="#8B54FF"/>
                                </g>
                                <defs>
                                <clipPath id="clip0_10099_272">
                                <rect width="14" height="16" fill="white" transform="matrix(1 0 0 -1 0.660156 16.668)"/>
                                </clipPath>
                                </defs>
                                </svg>
                                <p>25/6/2023</p>
                                </div>
                        </div>
                    </div>
                    <div class="df">
                        <img src="img/blog1.jpg" class="blogimg" alt="" srcset="">
                        <div class="df-c g-0">
                            <div class="title2">
                                How To Start Learn Online
                                Study From Your Home
                            </div>
                            <div class="df"><svg width="15" height="17" viewBox="0 0 15 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_10099_272)">
                                <path d="M4.16016 0.667969C4.47266 0.688802 4.63932 0.855469 4.66016 1.16797V2.66797H10.6602V1.16797C10.681 0.855469 10.8477 0.688802 11.1602 0.667969C11.4727 0.688802 11.6393 0.855469 11.6602 1.16797V2.66797H12.6602C13.2227 2.6888 13.6914 2.88672 14.0664 3.26172C14.4414 3.63672 14.6393 4.10547 14.6602 4.66797V14.668C14.6393 15.2305 14.4414 15.6992 14.0664 16.0742C13.6914 16.4492 13.2227 16.6471 12.6602 16.668H2.66016C2.09766 16.6471 1.62891 16.4492 1.25391 16.0742C0.878906 15.6992 0.68099 15.2305 0.660156 14.668V4.66797C0.68099 4.10547 0.878906 3.63672 1.25391 3.26172C1.62891 2.88672 2.09766 2.6888 2.66016 2.66797H3.66016V1.16797C3.68099 0.855469 3.84766 0.688802 4.16016 0.667969ZM13.6602 6.66797H1.66016V14.668C1.66016 14.9596 1.75391 15.1992 1.94141 15.3867C2.12891 15.5742 2.36849 15.668 2.66016 15.668H12.6602C12.9518 15.668 13.1914 15.5742 13.3789 15.3867C13.5664 15.1992 13.6602 14.9596 13.6602 14.668V6.66797ZM12.6602 3.66797H2.66016C2.36849 3.66797 2.12891 3.76172 1.94141 3.94922C1.75391 4.13672 1.66016 4.3763 1.66016 4.66797V5.66797H13.6602V4.66797C13.6602 4.3763 13.5664 4.13672 13.3789 3.94922C13.1914 3.76172 12.9518 3.66797 12.6602 3.66797Z" fill="#8B54FF"/>
                                </g>
                                <defs>
                                <clipPath id="clip0_10099_272">
                                <rect width="14" height="16" fill="white" transform="matrix(1 0 0 -1 0.660156 16.668)"/>
                                </clipPath>
                                </defs>
                                </svg>
                                <p>25/6/2023</p>
                                </div>
                        </div>
                    </div>
                    <div class="df">
                        <img src="img/blog1.jpg" class="blogimg" alt="" srcset="">
                        <div class="df-c g-0">
                            <div class="title2">
                                How To Start Learn Online
                                Study From Your Home
                            </div>
                            <div class="df"><svg width="15" height="17" viewBox="0 0 15 17" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <g clip-path="url(#clip0_10099_272)">
                                <path d="M4.16016 0.667969C4.47266 0.688802 4.63932 0.855469 4.66016 1.16797V2.66797H10.6602V1.16797C10.681 0.855469 10.8477 0.688802 11.1602 0.667969C11.4727 0.688802 11.6393 0.855469 11.6602 1.16797V2.66797H12.6602C13.2227 2.6888 13.6914 2.88672 14.0664 3.26172C14.4414 3.63672 14.6393 4.10547 14.6602 4.66797V14.668C14.6393 15.2305 14.4414 15.6992 14.0664 16.0742C13.6914 16.4492 13.2227 16.6471 12.6602 16.668H2.66016C2.09766 16.6471 1.62891 16.4492 1.25391 16.0742C0.878906 15.6992 0.68099 15.2305 0.660156 14.668V4.66797C0.68099 4.10547 0.878906 3.63672 1.25391 3.26172C1.62891 2.88672 2.09766 2.6888 2.66016 2.66797H3.66016V1.16797C3.68099 0.855469 3.84766 0.688802 4.16016 0.667969ZM13.6602 6.66797H1.66016V14.668C1.66016 14.9596 1.75391 15.1992 1.94141 15.3867C2.12891 15.5742 2.36849 15.668 2.66016 15.668H12.6602C12.9518 15.668 13.1914 15.5742 13.3789 15.3867C13.5664 15.1992 13.6602 14.9596 13.6602 14.668V6.66797ZM12.6602 3.66797H2.66016C2.36849 3.66797 2.12891 3.76172 1.94141 3.94922C1.75391 4.13672 1.66016 4.3763 1.66016 4.66797V5.66797H13.6602V4.66797C13.6602 4.3763 13.5664 4.13672 13.3789 3.94922C13.1914 3.76172 12.9518 3.66797 12.6602 3.66797Z" fill="#8B54FF"/>
                                </g>
                                <defs>
                                <clipPath id="clip0_10099_272">
                                <rect width="14" height="16" fill="white" transform="matrix(1 0 0 -1 0.660156 16.668)"/>
                                </clipPath>
                                </defs>
                                </svg>
                                <p>25/6/2023</p>
                                </div>
                        </div>
                    </div>

                </div>
                <div class="needhelp">
                    <div class="nhb df-c center">
                        <div class="title ta-c cw">Need Help? We Are
                            Here To Help You</div>
                        <div class="nhlogo">
                            <img src="img//Layer_2.png" class="layer" alt="" srcset="">
                        </div>
                        <div class="title cw">
                            You Get Your Courses
                        </div>
                        <div class="div">+216 51 717 722</div>
                        <div class="btn cw">Contact Us Now </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <div class="footer container center ">
      <div class="df fx-s">
          <div class="df-c">
              <img src="img/Layer_2.png" class="layer" alt="" srcset="">
          </div>
          <div class="df-c footer-box">
              <div class="title-bold">
                  Information
              </div>
              <p>
                  contact@3s-spring.tn
                  +216 51 717 722
                  Menzel Abderrahman Bizerte, 7035
                  Monday - Sunday : 9AM - 10PM
              </p>
          </div>
          <div class="df-c footer-box">
              <div class="title-bold">
                  links
              </div>
              <p class="df-c">
                  <a href="">home</a>
                  <a href="">about</a>
                  <a href="">service</a>
                  <a href="">career</a>
                  <a href="">contact us</a>
              </p>
          </div>
          <div class="df-c footer-box">
              <div class="title-bold">
                  quick links
              </div>
              <p class="df-c">
                  <a href="">Management & Consulting</a><a href="">Software Development</a><a href="">Digital Marketing</a><a href="">Data Manipulation</a>
              </p>
          </div>
          <div class="df-c footer-box">
              <div class="title-bold">
                  business
              </div>
              <p>
                Our company specialized in software
                development, project management, digital marketing and data science.                  
              </p>
          </div>
      </div>
  </div>
  <div class="copyright container">
      <div class="wrapper">
          <div class="df">© 2024 3S SPRING. All rights Reserved. <div>Terms & Conditions</div></div>
          <div class="df">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M15.7432 3.98403H17.9992V0.168036C16.9069 0.0544523 15.8094 -0.00162514 14.7112 3.58369e-05C11.4472 3.58369e-05 9.21524 1.99203 9.21524 5.64003V8.78402H5.53125V13.056H9.21524V24H13.6312V13.056H17.3032L17.8552 8.78402H13.6312V6.06003C13.6312 4.80003 13.9672 3.98403 15.7432 3.98403Z" fill="white"/>
                  </svg>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M15.7432 3.98403H17.9992V0.168036C16.9069 0.0544523 15.8094 -0.00162514 14.7112 3.58369e-05C11.4472 3.58369e-05 9.21524 1.99203 9.21524 5.64003V8.78402H5.53125V13.056H9.21524V24H13.6312V13.056H17.3032L17.8552 8.78402H13.6312V6.06003C13.6312 4.80003 13.9672 3.98403 15.7432 3.98403Z" fill="white"/>
                  </svg>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M15.7432 3.98403H17.9992V0.168036C16.9069 0.0544523 15.8094 -0.00162514 14.7112 3.58369e-05C11.4472 3.58369e-05 9.21524 1.99203 9.21524 5.64003V8.78402H5.53125V13.056H9.21524V24H13.6312V13.056H17.3032L17.8552 8.78402H13.6312V6.06003C13.6312 4.80003 13.9672 3.98403 15.7432 3.98403Z" fill="white"/>
                  </svg>
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M15.7432 3.98403H17.9992V0.168036C16.9069 0.0544523 15.8094 -0.00162514 14.7112 3.58369e-05C11.4472 3.58369e-05 9.21524 1.99203 9.21524 5.64003V8.78402H5.53125V13.056H9.21524V24H13.6312V13.056H17.3032L17.8552 8.78402H13.6312V6.06003C13.6312 4.80003 13.9672 3.98403 15.7432 3.98403Z" fill="white"/>
                  </svg>
                  
          </div>
      </div>
  </div>
  <script src="js/master.js"></script>
</body>
</html>